...
CREATE TABLE MachineUsers ( 
    MachineID INTEGER,
    UsersID INTEGER,
    PRIMARY KEY (MachineID, UsersID),
    FOREIGN KEY (MachineID) REFERENCES Machine (MachineID) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (UsersID) REFERENCES Users (UserID) ON DELETE CASCADE ON UPDATE CASCADE
);
...
INSERT INTO MachineUsers (MachineID, UsersID) VALUES 
    (1234, 1),
    (1235,1),
    (1236,3),
    (1237,4);
...
INSERT INTO PersonalData (UserID, FirstName, LastName, PhoneNumber, Ethnicitykey, Email) VALUES
    ...
    (1,'Jonny','Comes','2084532345',NULL,'Jcomes@collegeofidaho.edu');
...
INSERT INTO Response (ReponseID, ResponseDescription, ResponseDate, Status)
...
SELECT 
    ...
    U.Profession, 
    Rs.ReponseDate AS "Response_Date",
    Rs.ResponseDescription "Response_Description",
    Rs.Status
FROM Response as Rs
...
JOIN Users AS u ON u.UsersID = mu.UsersID
JOIN MachineUsers AS mu ON m.MachineID = mu.MachineID
JOIN PersonalDate AS pd ON u.UserID = pd.UserID;
...
JOIN MachineUsers AS MU ON MU.UsersID = U.UserID
